...
def test_loads_the_registered_final_withheld_datasets():
    ...
    assert {
        ...
        "final-withheld-acadia-terrain-v1",
        "final-withheld-baltimore-terrain-surface-v1",
        "final-evaluation-calibration-copernicus-glo30-v1",
    }
    for dataset in (
        dataset
        for dataset in registry["datasets"]
        if dataset["dataset_role"] == "final_withheld_validation"
    ):
        validate_dataset_manifest(dataset, registry)
    calibration = next(
        dataset
        for dataset in registry["datasets"]
        if dataset["dataset_id"] == "final-evaluation-calibration-copernicus-glo30-v1"
    )
    assert calibration["dataset_role"] == "calibration"
    assert calibration["purpose"] == "final_evaluation_calibration_only"
    assert calibration["independence"]["used_for_calibration"] is True
    assert calibration["independence"]["eligible_for_final_withheld_validation"] is False
    assert registry["protected_final_withheld_geographies"][0]["geographic_identity_id"] == "us-co-boulder"


def test_registered_artifact_paths_are_portable_external_paths():
    registry = load_dataset_registry(REGISTRY_PATH)
    ...
    assert "C:\\\\Users\\\\" not in serialized
    for dataset in registry["datasets"]:
        assert "shawn_d8kl69k" not in str(dataset)
